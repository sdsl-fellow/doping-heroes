import {bookSources,firstBookSource} from './book-progress.mjs';
...
export function StageBook({stage,save,rewardsReady,onComplete}:{stage:number;save:{readBooks:number[];readBookSources?:string[]};rewardsReady:boolean;onComplete:(sourceId:string)=>void}){
 ...
 const read=bookSources(save),activeSource=selected?.id??firstBookSource(stage),completed=read.includes(activeSource);
 ...
 if(sources.length&&!selected)return <section className="lecture-book" aria-label="낡은 책 자료 목차">
  ...
  <div style={{display:'grid',gap:12}}>{sources.map(source=><button key={source.id} style={{width:'100%',textAlign:'left',padding:16,whiteSpace:'normal'}} onClick={()=>{setPage(0);setSourceId(source.id);}}>
   ...
   <span style={{display:'block',marginTop:8,fontSize:14}}>{source.pages.length}장 · {read.includes(source.id)?'완독 ✓':'읽기'}</span>
  </button>)}</div>

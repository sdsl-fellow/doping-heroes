...
export function walkable(x,y){return x>=32&&x<=1504&&y>=340&&y<=1000&&corridors.some(([a,b,c,d,w])=>distance(x,y,a,b,c,d)<w);}
...
function nearest(x,y){let id=valid[0],best=Infinity;for(const v of valid){const d=(v%cols*cell+8-x)**2+(Math.floor(v/cols)*cell+8-y)**2;if(d<best){best=d;id=v;}}return id;}
export function route(x,y,tx,ty){const start=nearest(x,y),end=nearest(tx,ty),queue=[start],previous=new Map([[start,-1]]),allowed=new Set(valid);for(let n=0;n<queue.length;n++){const v=queue[n];if(v===end)break;for(const next of [v-1,v+1,v-cols,v+cols]){if(!allowed.has(next)||previous.has(next)||Math.abs(v%cols-next%cols)>1)continue;previous.set(next,v);queue.push(next);}}if(!previous.has(end))return [];const result=[];for(let v=end;v!==start;v=previous.get(v)){result.unshift({x:v%cols*cell+8,y:Math.floor(v/cols)*cell+8});}return result;}

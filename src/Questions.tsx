...
export type QuestionRecord={id:string;stage:number;subject:string;question:string;status:string;createdAt:string;answer:string;answeredBy:string;answeredAt:string};
...
export function MyQuestions({token,onBusy,onBack}:Pick<Props,'token'|'onBusy'|'onBack'>){
 ...
 return <section className="qa-panel"><p>최근 질문부터 표시합니다. 답변을 확인하려면 새로고침을 눌러주세요.</p><button disabled={busy} onClick={()=>void load()}>새로고침</button>
  ...
  {items.map(q=><article className="qa-card" key={q.id}><small>{q.status} · {q.stage?stageDefinitions[q.stage-1]?.title:'일반 질문'}</small><h3>{q.subject}</h3><small>질문 등록: {q.createdAt}</small><p className="qa-text">{q.question}</p>{q.answer?<div className="qa-answer"><strong>{q.answeredBy}의 답변</strong><p className="qa-text">{q.answer}</p>{q.answeredAt&&<small>답변 수정: {q.answeredAt}</small>}</div>:<p>교수·조교의 답변을 기다리고 있습니다.</p>}</article>)}

 ...
 class Campus extends Phaser.Scene{
  ...
  create(){
   ...
   if(area==='village'){
    ...
    this.boat=this.add.image(768,965,'journey-props',2).setDisplaySize(185,185).setDepth(950);
  ...
  openGate(){
   ...
   this.tweens.add({targets:key,x:768,y:823,duration:this.motion(600),onComplete:()=>{
    this.tweens.add({targets:key,angle:90,duration:this.motion(450),onComplete:()=>{
     ...
     this.tweens.add({targets:this.player,y:995,delay:this.motion(500),duration:this.motion(1100),onUpdate:()=>{this.player!.setDepth(this.player!.y);this.label?.setPosition(this.player!.x,this.player!.y-108);},onComplete:()=>{
      this.player!.setScale(.95).setDepth(980);this.boat!.setDepth(979);this.gate!.setDepth(850);
      const wake=this.add.ellipse(768,984,90,18,0xc7f4ff,.4).setDepth(978);
      live.current.onError('모험 대륙으로 출항합니다!');
      this.tweens.add({targets:[this.boat,this.player,this.label,wake].filter(Boolean),y:'+=180',duration:this.motion(2200),ease:'Sine.easeInOut',onComplete:()=>{live.current.onBusy(false);live.current.onTravel();}});
     }});
    }});
   }});
  }

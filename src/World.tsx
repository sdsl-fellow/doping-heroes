...
export function World({rootAccount,character,name,completed,area,arrival,destination,active,onTalk,onBook,onMiniGame,onBusy,onTravel,onEnterStage,onPosition,onError}:{rootAccount:boolean;character:Character;name:string;completed:number[];area:Save['area'];arrival:number|null;destination:number|null;active:boolean;onBook:()=>void;onMiniGame:()=>void;onBusy:(busy:boolean)=>void;onTravel:()=>void;onEnterStage:(index:number)=>void;onTalk:(i:number)=>void;onPosition:(x:number,y:number)=>void;onError:(text:string)=>void}){
 const root=useRef<HTMLDivElement>(null),live=useRef({rootAccount,character,name,completed,area,arrival,destination,active,onTalk,onBook,onMiniGame,onBusy,onTravel,onEnterStage,onPosition,onError});live.current={rootAccount,character,name,completed,area,arrival,destination,active,onTalk,onBook,onMiniGame,onBusy,onTravel,onEnterStage,onPosition,onError};
 useEffect(()=>{
 ...
 class Campus extends Phaser.Scene{
  ...
  update(time:number,delta:number){
   ...
   if(!this.player)return;const tutorialDone=[0,1,2].every(id=>props.completed.includes(id));this.speech?.setVisible(tutorialDone);if(area==='village')this.markers[0]?.setVisible(!tutorialDone);this.markers.forEach((m,i)=>m.setText(area==='adventure'?(props.completed.includes(stageDefinitions[i].questId)?'✓ ':stageUnlocked(props.completed,i,props.rootAccount)?'':'🔒 ')+stageDefinitions[i].title:stage?(props.completed.includes(stage.questId)?'✓':'!'):i===0?([0,1,2].every(q=>props.completed.includes(q))?'✓':'!'):i===1?'$':'◈'));

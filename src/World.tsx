...
export function World({character,name,completed,area,arrival,destination,active,onTalk,onTravel,onEnterStage,onPosition,onError}:{character:Character;name:string;completed:number[];area:Save['area'];arrival:number|null;destination:number|null;active:boolean;onTravel:()=>void;onEnterStage:(index:number)=>void;onTalk:(i:number)=>void;onPosition:(x:number,y:number)=>void;onError:(text:string)=>void}){
 ...
 class Campus extends Phaser.Scene{
  ...
  preload(){if(stage)this.load.spritesheet('campus',info.image,{frameWidth:362,frameHeight:362});else this.load.image('campus',info.image);this.load.on('loaderror',()=>live.current.onError('맵을 불러오지 못했습니다. 새로고침해 주세요.'));}
  ...
  create(){
   this.add.image(0,0,'campus',stage?index:undefined).setOrigin(0).setDisplaySize(info.width,info.height);this.cameras.main.setBounds(0,0,info.width,info.height);this.cameras.main.setScroll(0,150);
   ...
   places.forEach((p,i)=>{if(area==='adventure'){this.add.ellipse(p.x,p.y,90,48,0x4fcee4,.3).setStrokeStyle(4,0xffe39a).setDepth(p.y);this.markers.push(this.add.text(p.x,p.y-42,stageDefinitions[i].title,{fontSize:'17px',color:'#fff3c3',backgroundColor:'#163e49dd',align:'center',wordWrap:{width:190},padding:{x:7,y:6}}).setOrigin(.5,1).setDepth(1500));this.add.text(p.x,p.y,'관문',{fontSize:'18px',color:'#fff8df'}).setOrigin(.5).setDepth(1500);this.add.zone(p.x,p.y-20,120,100).setInteractive().setDepth(1600).on('pointerdown',()=>{if(live.current.active)this.go(p.x,p.y,i);});return;}const c={...defaultCharacter,gender:i===2?'female':'male',body:i===1?'sturdy':'agile',hair:i===0?'bangs':i===1?'bedhead':'bob',hairColor:i===0?'#dae0e5':i===1?'#77452f':'#b298d1',outfitColor:i===0?'#e6e9e5':i===1?'#438674':'#695c98'} as Character;this.sprite(c,'npc-'+i,p.x,p.y).catch(e=>live.current.onError(e.message));this.markers.push(this.add.text(p.x,p.y-109,'!',{fontFamily:'sans-serif',fontStyle:'bold',fontSize:'32px',color:'#ffe895',stroke:'#263145',strokeThickness:6}).setOrigin(.5).setDepth(1500));this.add.text(p.x,p.y+5,names[i],{fontSize:'16px',color:'#fff8dd',backgroundColor:'#152c36cc',padding:{x:8,y:4}}).setOrigin(.5,0).setDepth(1500);this.add.zone(p.x,p.y-40,100,120).setInteractive({useHandCursor:true}).setDepth(1600).on('pointerdown',()=>{if(live.current.active)this.go(p.x,p.y+20,i);});});
   if(area==='village')for(const t of [{x:768,y:325,text:'세미 어드벤처'},{x:230,y:420,text:'도너 상점'},{x:1330,y:485,text:'실리콘 결정 동굴'}])this.add.text(t.x,t.y,t.text,{fontSize:'20px',color:'#fff4d4',stroke:'#1a3547',strokeThickness:6}).setOrigin(.5).setDepth(1500);
   this.add.text(info.exit.x,area==='village'?865:135,area==='village'?'↓ 남쪽 다리 · 모험 대륙':stage?'↑ 모험 대륙으로 돌아가기':'↑ 세미 마을로 돌아가기',{fontSize:'18px',color:'#fff4d4',backgroundColor:'#193d43dd',padding:{x:12,y:10}}).setOrigin(.5).setDepth(1500).setInteractive().on('pointerdown',()=>{if(live.current.active)this.go(info.exit.x,info.exit.y,null);});
  ...
  update(time:number,delta:number){
   ...
   if(!this.path.length){this.destination?.setVisible(false);if(this.pending!==null){const i=this.pending;this.pending=null;if(Math.hypot(this.player.x-places[i].x,this.player.y-places[i].y)<100)interact(i);}}
   if(area==='adventure'&&manual){const gate=gatewayLocations.findIndex(p=>Math.hypot(this.player!.x-p.x,this.player!.y-p.y)<20);if(gate>=0){this.player.y+=65;this.path=[];props.onEnterStage(gate);}}
   const exiting=area==='village'?this.player.y>925&&Math.abs(this.player.x-768)<55:this.player.y<110&&Math.abs(this.player.x-info.exit.x)<60;
   if(exiting){this.player.y=area==='village'?890:170;this.path=[];props.onTravel();}

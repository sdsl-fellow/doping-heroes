...
export function SiliconLibrary(){const [tab,setTab]=useState('terms'),[search,setSearch]=useState('');const rows=(tab==='terms'?glossary:faqs).filter(r=>r.join(' ').toLowerCase().includes(search.toLowerCase()));return <section><div className="choices"><button aria-pressed={tab==='terms'} onClick={()=>setTab('terms')}>반도체 용어</button><button aria-pressed={tab==='faq'} onClick={()=>setTab('faq')}>궁금한 질문 · FAQ</button></div><label>찾아보기<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="도핑, 정공, 전도도…"/></label><div className="silicon-library">{rows.map(([title,meaning])=><details key={title}><summary>{title}</summary><p>{meaning}</p></details>)}{!rows.length&&<p>일치하는 항목이 없습니다.</p>}</div></section>;}

...
export function InventoryControls({character,completed,purchased,quantities={},onEquip,onUse,saveControl,busy=false}:{character:Character;completed:number[];purchased:string[];quantities?:Record<string,number>;onEquip:(c:Character)=>void;onUse:(id:string)=>void;saveControl?:import('react').ReactNode;busy?:boolean}){
 ...
 return <button key={item.id} type="button" aria-pressed={tool?undefined:selected} disabled={busy||(tool&&(!isConsumable(item.id)||quantity<1))} onClick={()=>tool?onUse(item.id):onEquip({...character,[slot]:item.id})}><ItemIcon id={item.id}/><span className="catalog-code">{item.code}</span><strong>{item.name}</strong><small>{tool?(isConsumable(item.id)?quantity+'개 · 사용':item.id==='T01'?'관문에서 사용':item.id==='T02'?'스테이지에서 읽기':'수집품'):selected?'선택 중':'선택하기'}</small>{tool&&<small>{itemDescription(item)}</small>}</button>;
 })}{slot!=='tool'&&<button type="button" key={'none-'+slot} disabled={busy} aria-pressed={character[slot as keyof Character]==='none'} onClick={()=>onEquip({...character,[slot]:'none'})}><span className="catalog-icon empty-equipment">—</span><strong>없음</strong><small>{character[slot as keyof Character]==='none'?'선택 중':'미착용'}</small></button>}</div></fieldset>)}</div>;
}

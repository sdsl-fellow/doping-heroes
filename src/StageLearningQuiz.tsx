...
export function StageLearningQuiz({load,request,answered,onSolved,onComplete,onBusy,practice=false}:Props){
 ...
 useEffect(()=>{let alive=true;setBusy(true);onBusy(true);setError('');
  loadRef.current(retry>0).then(data=>{
   if(!data.questions?.length||data.questions.some(q=>!q.sessionId||!q.questionId||!q.options))throw new Error('스테이지 문제를 준비하지 못했습니다.');
 ...
 const check=async()=>{
  ...
  try{const result=await request('learningAnswer',{sessionId:q.sessionId,optionId:choice});
 ...
 return <section>
  {questions.length>0&&<>
   ...
   {q&&<><h3>{q.title}</h3><p className="question">{q.question}</p>
    <div className="answers">{q.options?.map((option,i)=><button key={option.id} disabled={busy||solved} aria-pressed={choice===option.id} onClick={()=>{setChoice(option.id);setFeedback('');}}><b>{i+1}</b>{option.text}</button>)}</div>

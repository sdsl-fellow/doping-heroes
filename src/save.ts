...
export type Save={version:3;studentId:string;name:string;character:Character;completed:number[];readBooks:number[];doping:number;type:'n'|'p';coins:number;purchased:string[];area:'village'|'adventure'|`stage-${number}`};

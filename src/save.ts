...
export type Save={version:3;item_schema?:number;stage_layout?:number;studentId:string;name:string;character:Character;completed:number[];readBooks:number[];readBookSources?:string[];puzzle_completed:number[];doping:number;type:'n'|'p';coins:number;quantities?:Record<string,number>;purchased:string[];area:'village'|'adventure'|`stage-${number}`};

...
export type Save={version:3;studentId:string;name:string;character:Character;completed:number[];readBooks:number[];fetPuzzleCompleted?:boolean;doping:number;type:'n'|'p';coins:number;quantities?:Record<string,number>;purchased:string[];area:'village'|'adventure'|`stage-${number}`};

...
export function TranslationQuiz({request,onBusy,prepared,onRound}:{request:TranslationRequest;onBusy:(value:boolean)=>void;prepared?:Promise<TranslationRound>|null;onRound?:(round:TranslationRound)=>void}){
 ...
 useEffect(()=>{if(!prepared)return;let alive=true;setBusy(true);onBusy(true);
  prepared.then(r=>{if(alive)setRound(r);}).catch(e=>{if(alive)setError(e instanceof Error?e.message:'문제를 다시 불러오세요.');}).finally(()=>{if(alive){setBusy(false);onBusy(false);}});
  return()=>{alive=false;onBusy(false);};
 },[prepared]);
 const start=()=>void run(async()=>{if(!roundId.current)roundId.current=crypto.randomUUID();const next=await request('translationStart',{roundId:roundId.current});setRound(next);onRound?.(next);setIndex(0);setSelected('');roundId.current='';});
 ...
 return <section className="translation-quiz" aria-busy={busy}>
  ...
  {q&&<><div className="translation-meta"><span>{index+1} / {round!.questions.length} · {q.kind==='term'?'용어':'문장'} 번역</span><small>문제 {q.id}</small></div><blockquote lang="en">{q.english}</blockquote><div className="translation-options">{q.options.map((o,i)=><button key={o.id} disabled={busy||!!result} aria-pressed={selected===o.id} className={result?(o.id===result.correctOption?'translation-correct':o.id===result.selected?'translation-wrong':''):selected===o.id?'translation-selected':''} onClick={()=>setSelected(o.id)}><b>{i+1}</b><span>{o.text}</span></button>)}</div>
   {!result&&<button className="primary" disabled={busy||!selected} onClick={()=>void run(async()=>{const next=await request('translationAnswer',{roundId:round!.id,questionId:q.id,optionId:selected});setRound(next);onRound?.(next);})}>{busy?'정답 확인 및 저장 중…':'정답 확인'}</button>}
   {result&&<div className="translation-feedback" role="status"><strong>{result.correct?'정답입니다!':'정답을 확인해 보세요.'}</strong><p>정답: {result.correctText}</p><p>{result.explanation}</p><p>{result.correct?`${result.repeat?'복습':'최초 정답'} 보상 · 도핑 +${scientific(result.dose)} cm⁻³ · ${result.coins}코인`:'보상 없음 · 다음 회차에서 다시 도전할 수 있습니다.'}</p><small>출처: {result.sourceId==='SE02-ATOMS-2026'?'[SE] 02. Atoms and Electrons':result.sourceTitle} · p. {result.sourcePage}</small><button className="primary" disabled={busy} onClick={()=>{setIndex(i=>i+1);setSelected('');}}>{index===round!.questions.length-1?'결과 보기':'다음 문제'}</button></div>}
  </>}
  {finished&&<><h3>5문제 완료</h3><p>정답 {Object.values(round!.results).filter(r=>r.correct).length} / 5 · 획득 {Object.values(round!.results).reduce((n,r)=>n+r.coins,0)}코인</p><p>{round!.storageMode==='student-question'?'학습 기록과 보상이 저장되었습니다. 집계 시트에는 순차 반영됩니다.':'학습 기록과 보상은 서버에 저장되었습니다.'}</p><button className="primary" disabled={busy} onClick={start}>{busy?'다음 회차 준비 중…':'새로운 5문제 도전'}</button></>}

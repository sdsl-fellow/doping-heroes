...
const context=vm.createContext({PropertiesService:{getScriptProperties:()=>({getProperty:k=>k==='ROOT_STUDENT_ID'?'000001':k==='ROOT_NAME'?'관리자':''})},Utilities:{formatDate(date,zone){
...
test('sheet items match the inventory for defaults, quests, loot, tools and root',()=>{
 ...
 assert.equal(context.studentRow_('000001','관리자',{name:'관리자',studentId:'000001',completed:[],purchased:[],coins:0},1,'','','','')[5].split(',').length,60);
});

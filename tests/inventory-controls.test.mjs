...
const {InventoryControls}=await import('data:text/javascript;base64,'+Buffer.from(bundle.outputFiles[0].text).toString('base64'));
...
for(const species of ['dog','cat'])test(`${species}: all 50 owned wearables can be clicked and equip their slot`,()=>{
 ...
 let equipped;
 const rendered=InventoryControls({character,completed:[],purchased:catalog.map(i=>i.id),quantities:{T04:1},onEquip:c=>{equipped=c;},onUse:()=>{}});
 ...
 for(const slot of ['outfit','shoes','hat','weapon','accessory']){
  controls.get('none-'+slot).onClick();assert.equal(equipped[slot],'none');
 }
});

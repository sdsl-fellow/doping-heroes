...
test('puzzle indices use the current stage order',()=>{
 assert.deepEqual(puzzleIds({fetPuzzleCompleted:true}),[]);
...
test('current map quest IDs serialize without old stage conversion',()=>{
 assert.deepEqual(toStoredSave({completed:[11]}).stage_completed,[7]);
 assert.deepEqual(toStoredSave({completed:[10]}).stage_completed,[6]);
 assert.deepEqual(toStoredSave({stage_completed:[8],tutorial_completed:[]}).stage_completed,[8]);
 assert.deepEqual(fromStoredSave({completion_schema:2,stage_completed:[1]}).completed,[4]);
});
test('obsolete column removal backs up first and preserves all other cells',()=>{
 ...
 const save=ctx.normalizeSave_({completed:[0,1,2],puzzle_completed:[6]},'20260001','학생');

...
test('FET legacy completion belongs to Stage 11, index 10',()=>{
 assert.deepEqual(puzzleIds({fetPuzzleCompleted:true}),[10]);
 assert.deepEqual(puzzleIds({fetPuzzleCompleted:false}),[]);
 for(let stage=0;stage<12;stage++){
  const save=toStoredSave({completed:[],puzzle_completed:[stage]});
 ...
 assert.deepEqual(puzzleIds({puzzle_completed:[0,11,11,12,-1,'2']}),[0,11]);
});
test('actual map quest order and v5 arrays migrate without progress loss',()=>{
 assert.deepEqual(toStoredSave({completed:[11]}).stage_completed,[1]);
 assert.deepEqual(toStoredSave({completed:[10]}).stage_completed,[10]);
 assert.deepEqual(toStoredSave({stage_completed:[8],tutorial_completed:[]}).stage_completed,[1]);
...
test('obsolete column removal backs up first and preserves all other cells',()=>{
 ...
 assert.deepEqual(JSON.parse(row[6]).puzzle_completed,[10]);
});
test('save API uses the shifted revision and credential columns',()=>{
 ...
 assert.deepEqual(JSON.parse(JSON.stringify(response.student.save.puzzle_completed)),[10]);

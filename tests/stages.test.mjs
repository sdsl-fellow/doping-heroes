...
test('continent arrival is north of portals with a downward route; ocean shoreline blocks water',()=>{
 const start=arrivalPoint('adventure');assert.ok(start.y<Math.min(...gatewayLocations.map(g=>g.y)));const path=route(start.x,start.y,768,330,'adventure');assert.ok(path.length);assert.ok(path.at(-1).y>start.y);assert.equal(walkable(768,800,'stage-6'),false);
});
...
test('stage artwork water and chasm remain blocked',()=>{assert.equal(walkable(500,350,'stage-8'),false);assert.equal(walkable(768,800,'stage-6'),false);assert.equal(walkable(506,470,'stage-12'),true);});

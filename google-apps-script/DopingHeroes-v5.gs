...
const API_VERSION = 5;
const RELEASE_LABEL = 'completion-split-5';
    ...
    if (!Number.isInteger(baseRevision) || baseRevision !== currentRevision) {
      return {
        ...
        student: {
          ...
          save: toStoredSave(parseStoredSave_(values[7], auth.studentId, String(values[1]))),
...
function migrateCompletionFields() {
  ...
  try {
    ...
    for (let row = 2; row <= sheet.getLastRow(); row++) {
      const values = sheet.getRange(row, 8, 1, 2).getValues()[0];
      ...
      sheet.getRange(row, 8, 1, 2).setValues([[serialized, (Number(values[1]) || 1) + 1]]);
      changed++;
    }
    SpreadsheetApp.flush();
...
function repairInventoryAndSeoulTime() {
  ...
  try {
    ...
    for (let row = 2; row <= students.getLastRow(); row++) {
      ...
      if (!values[0]) continue;
      let save;
      try { save = fromStoredSave(JSON.parse(String(values[7] || '{}'))); } catch (_) { continue; }
      ...
      students.getRange(row, 7).setValue(inventoryIds(save).join(','));
      repaired++;
    }
    [[students, [10,11]], [ss.getSheetByName(STAGES_SHEET), [3]], [ss.getSheetByName(AUDIT_SHEET), [1]]].forEach(function (entry) {
      const sheet = entry[0];

...
const API_VERSION = 9;
const RELEASE_LABEL = 'v9-curriculum-maps';
...
const legacyStageQuestIds = [3,11,4,5,6,7,8,9,12,13,10,14];
const oldToNewStage = [0,7,1,2,8,3,4,9,11,5,6,10];
function stageList(values){return [...new Set((Array.isArray(values)?values:[]).filter(n=>Number.isInteger(n)&&n>=0&&n<12))].sort((a,b)=>a-b);}
function stageIndices(values,layout){return stageList(stageList(values).map(n=>Number(layout)>=3?n:oldToNewStage[n]));}
...
function puzzleIds(save){return stageIndices(Array.isArray(save?.puzzle_completed)?save.puzzle_completed:save?.fetPuzzleCompleted===true?[10]:[],save?.stage_layout);}
...
// One-time curriculum reorder, idempotent; preserves balances and item ownership.
function migrateStageLayout() {
  return migrateItemCatalog().replace('아이템 ID 변환', '스테이지 번호 및 완료·완독·퍼즐 기록 변환');
}

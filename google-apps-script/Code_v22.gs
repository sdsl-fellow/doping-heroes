...
const RELEASE_LABEL = 'v22-question-categories';
...
function learningQuizBank_(stage){
 ...
 return learningRows_(name,QUIZ_CONTENT_HEADERS).filter(q=>stageReleasedValue_(q.active)).map(q=>{
  ...
  if(!/^[A-Za-z0-9_-]{1,80}$/.test(id)||seen.has(id)||!String(q.question).trim()||options.length<2||!options.some(o=>o.id===correct)||!Number.isFinite(Number(q.rewardDose))||Number(q.rewardDose)<0||!Number.isInteger(Number(q.rewardCoins))||Number(q.rewardCoins)<0)throw apiError_('CONTENT_QUESTION',name+' 문제·보기·정답·보상을 확인하세요.');
...
function stageQuizAnswer_(auth,session,request,cache){
 ...
 const correct=request.optionId===q.correctOption||batch.answered.includes(q.questionId);
...
function learningContentAction_(request){
 ...
 if(request.kind==='stageQuiz'){
  ...
  const questions=translationShuffle_(bank).map(q=>{
   ...
   return {sessionId:id,questionId:q.questionId,title:String(q.title),question:String(q.question),options:q.options};
  });

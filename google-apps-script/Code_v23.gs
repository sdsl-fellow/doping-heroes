...
const RELEASE_LABEL = 'v23-stage-numeric-quiz';
...
const QUIZ_NUMERIC_HEADERS=['questionType','numericAnswer','answerScale','answerUnit','tolerance','commonInstruction'];
...
function learningRows_(name,headers,optionalHeaders=[]){
 ...
 const extra=optionalHeaders.filter(h=>actual.includes(h));
 const n=sheet.getLastRow()-1;return n<1?[]:sheet.getRange(2,1,n,width).getValues().filter(r=>r.some(v=>v!=='')).map(r=>Object.fromEntries([...headers.map((h,i)=>[h,r[indices[i]]]),...extra.map(h=>[h,r[actual.indexOf(h)]]) ]));
}
...
function learningQuizBank_(stage){
 ...
 return learningRows_(name,QUIZ_CONTENT_HEADERS,stage===1?QUIZ_NUMERIC_HEADERS:[]).filter(q=>stageReleasedValue_(q.active)).map(q=>{
  ...
  const numeric=stage===1&&String(q.questionType).trim().toLowerCase()==='numeric';
  if(!/^[A-Za-z0-9_-]{1,80}$/.test(id)||seen.has(id)||!String(q.question).trim()||(!numeric&&(options.length<2||!options.some(o=>o.id===correct)))||!Number.isFinite(Number(q.rewardDose))||Number(q.rewardDose)<0||!Number.isInteger(Number(q.rewardCoins))||Number(q.rewardCoins)<0)throw apiError_('CONTENT_QUESTION',name+' 문제·보기·정답·보상을 확인하세요.');
  if(numeric&&(!Number.isFinite(Number(q.numericAnswer))||String(q.numericAnswer).trim()===''||!Number.isInteger(Number(q.answerScale))||Number(q.answerScale)<0||Number(q.answerScale)>30||!Number.isFinite(Number(q.tolerance))||Number(q.tolerance)<0||!String(q.answerUnit).trim()||!String(q.commonInstruction).trim()))throw apiError_('CONTENT_QUESTION',name+' 숫자형 문제의 정답·지수·단위·허용 오차를 확인하세요.');
  seen.add(id);return {...q,questionId:id,questionType:numeric?'numeric':'choice',correctOption:correct,options,dose:Number(q.rewardDose),coins:Number(q.rewardCoins),...(numeric?{numericAnswer:Number(q.numericAnswer),answerScale:Number(q.answerScale),tolerance:Number(q.tolerance)}:{})};
 });});
}
...
function setupLearningContentV16(){
 ...
 try{
  ...
  for(let n=1;n<=12;n++){
   const suffix=String(n).padStart(2,'0');learningRows_('Quiz_'+suffix,QUIZ_CONTENT_HEADERS,n===1?QUIZ_NUMERIC_HEADERS:[]);learningRows_('Translation_'+suffix,TRANSLATION_HEADERS);learningRows_('Experiment_'+suffix,EXPERIMENT_CONTENT_HEADERS);
  }
...
function stageQuizAnswer_(auth,session,request,cache){
 const key='learning-stage-batch:'+session.batchId,raw=cache.get(key);
 if(!raw)throw apiError_('CONTENT_EXPIRED','문제 준비 시간이 만료됐습니다. 창을 다시 열어주세요.');
 const batch=JSON.parse(raw),q=session.question;
 if(batch.studentId!==auth.studentId||batch.stage!==session.stage||batch.entries[q.questionId]!==request.sessionId)throw apiError_('FORBIDDEN','다른 학생의 문제입니다.');
 let answerCorrect=false;
 if(q.questionType==='numeric'){
  const raw=String(request.numericValue??'').trim();
  if(!/^(?:\d+(?:\.\d+)?|\.\d+)$/.test(raw)||raw.length>24)throw apiError_('CONTENT_ANSWER','지수와 단위를 제외한 숫자만 입력해주세요.');
  answerCorrect=Math.abs(Number(raw)-q.numericAnswer)<=q.tolerance+1e-9;
 }else{
  if(!q.options.some(o=>o.id===request.optionId))throw apiError_('CONTENT_ANSWER','보기를 선택해주세요.');
  answerCorrect=request.optionId===q.correctOption;
 }
 const correct=answerCorrect||batch.answered.includes(q.questionId);
 if(correct&&!batch.answered.includes(q.questionId)){
  batch.answered.push(q.questionId);
  cache.put(key,JSON.stringify(batch),1800);
 }
 const stageComplete=batch.answered.length===Object.keys(batch.entries).length;
 return {ok:true,content:{questionId:q.questionId,correct,explanation:correct?String(q.explanation):'다시 생각해 보세요.',solved:batch.answered.length,total:Object.keys(batch.entries).length,stageComplete,...(stageComplete?{dose:batch.rewardDose,coins:batch.rewardCoins}:{})}};
}
function learningContentAction_(request){
 ...
 if(request.action==='learningAnswer'){
  ...
  if(session.kind==='stage-quiz')return stageQuizAnswer_(auth,session,request,cache);
 ...
 if(request.kind==='stageQuiz'){
  if(stage===0)throw apiError_('CONTENT_STAGE','스테이지 퀴즈는 관문 안에서 시작하세요.');
  const bank=learningQuizBank_(stage);
  if(!bank.length)throw apiError_('CONTENT_EMPTY','활성 문제가 없습니다. 관리자가 콘텐츠 시트를 확인해야 합니다.');
  const batchId=Utilities.getUuid(),entries={};
  const questions=translationShuffle_(bank).map(q=>{
   const id=Utilities.getUuid();entries[q.questionId]=id;
   cache.put('learning-session:'+id,JSON.stringify({studentId:auth.studentId,kind:'stage-quiz',stage,batchId,question:q}),1800);
   return {sessionId:id,questionId:q.questionId,title:String(q.title),question:String(q.question),questionType:q.questionType,options:q.options,...(q.questionType==='numeric'?{answerScale:q.answerScale,answerUnit:String(q.answerUnit),commonInstruction:String(q.commonInstruction)}:{})};
  });
  cache.put('learning-stage-batch:'+batchId,JSON.stringify({studentId:auth.studentId,stage,entries,answered:[],rewardDose:bank[0].dose,rewardCoins:bank[0].coins}),1800);
  return {ok:true,content:{batchId,questions}};
 }
...
const QUESTION_CATEGORIES=['일반 질문','제안','기타'];
...
function ensureQuestionCategoryColumn_(sheet){
 const width=sheet.getLastColumn(),head=width?sheet.getRange(1,1,1,width).getValues()[0].map(String):[];
 // Keep the existing column positions and historical stage values intact.
 if(!head.includes('category'))sheet.getRange(1,width+1).setValue('category');
}
function questionPublic_(q){
 const published=String(q.status).trim()==='답변완료'&&String(q.answer||'').trim().length>0;
 return {id:String(q.questionId),stage:Number(q.stage)||0,category:QUESTION_CATEGORIES.includes(String(q.category))?String(q.category):'일반 질문',subject:String(q.subject||''),question:String(q.question||''),status:published?'답변완료':'답변대기',createdAt:String(q.createdAt||''),answer:published?String(q.answer):'',answeredBy:published?String(q.answeredBy||'교수·조교'):'',answeredAt:published?String(q.answeredAt||''):''};
}
...
function questionsAction_(request){
 ...
 const subject=questionText_(request.subject,80,'제목'),question=questionText_(request.question,2000,'질문 내용');
 const category=request.category===undefined?'일반 질문':request.category;
 if(typeof category!=='string'||!QUESTION_CATEGORIES.includes(category))throw apiError_('QA_INPUT','질문 분류를 확인해주세요.');
 // Older game clients still send stage; preserve that value until their UI updates.
 const stage=request.category===undefined?Number(request.stage):0;
 if(!Number.isInteger(stage)||stage<0||stage>12)throw apiError_('QA_INPUT','질문 스테이지를 확인해주세요.');
 const lock=LockService.getScriptLock();lock.waitLock(10000);
 try{
  const sheet=questionsSheet_();
  questionTable_(sheet); // Validate before adding a new column to the Questions tab.
  ensureQuestionCategoryColumn_(sheet);
  const table=questionTable_(sheet),existing=table.rows.find(q=>String(q.questionId)===id);
  ...
  const q={questionId:id,studentId:auth.studentId,studentName:name,stage,category,subject,question,answer:'',answeredBy:'',status:'답변대기',createdAt:koreaTimestamp_(),answeredAt:''};
